...
void gst_libcamera_set_exposure(ControlList &controls, const gint value)
{
	controls.set(controls::ExposureTime, value);
}

void gst_libcamera_set_ev(ControlList &controls, const gint value)
{
        controls.set(libcamera::controls::ExposureValue, value);
}


void gst_libcamera_set_shutter_limit(ControlList &controls, const gint value)
{
	controls.set(controls::AeMaxShutter, value);
}

void gst_libcamera_set_gain(ControlList &controls, const gint value)
{
	controls.set(controls::AnalogueGain, value);
}

void gst_libcamera_set_brightness(ControlList &controls, const float value)
{
	controls.set(controls::Brightness, value);
}

void gst_libcamera_set_contrast(ControlList &controls, const float value)
{
	controls.set(controls::Contrast, value);
}

void gst_libcamera_set_saturation(ControlList &controls, const float value)
{
	controls.set(controls::Saturation, value);
}

void gst_libcamera_set_sharpness(ControlList &controls, const float value)
{
	controls.set(controls::Sharpness, value);
}

void gst_libcamera_set_focus(ControlList &controls, const float value)
{
	/*
	float f = 0.0;
	...
	"Setting LensPosition: " << f);
	controls_.set(controls::LensPosition, f);
	*/
	//controls.set(controls::AfModeManual, value);
	controls.set(controls::LensPosition, value);
}

void gst_libcamera_set_digital_gain(const gint value)
{
	std::string dev("/dev/v4l-subdev0");
	int fd = open(dev.c_str(), O_RDWR, 0);
	v4l2_control ctrl1 { V4L2_CID_DIGITAL_GAIN, value };
	if (xioctl(fd, VIDIOC_S_CTRL, &ctrl1))
		GST_ERROR("WARNING: Unable to set digital gain");
    close(fd);
}

void gst_libcamera_set_hdr()
{
	bool ok = false;
	for (int i = 0; i < 4 && !ok; i++)
	{
		...
		if (fd < 0)
			continue;
		v4l2_control ctrl { V4L2_CID_WIDE_DYNAMIC_RANGE, TRUE };
		ok = !xioctl(fd, VIDIOC_S_CTRL, &ctrl);
		close(fd);
	}
	if (!ok)
		GST_ERROR("WARNING: Unable to set HDR mode");
}
					       ...
					       GstStructure *element_caps)
{
	...
	//initCtrls.set(controls::ExposureTime,2000);
        //initCtrls.set(controls::LensPosition,4);
        //initCtrls.set(controls::Saturation, 0);
	//initCtrls.set(controls::AnalogueGain, 2);
        initCtrls.set(controls::draft::NoiseReductionMode, controls::draft::NoiseReductionModeEnum::NoiseReductionModeOff);
...
/*
	std::string dev("/dev/v4l-subdev0");
	int fd = open(dev.c_str(), O_RDWR, 0);
	//v4l2_control ctrl { V4L2_CID_WIDE_DYNAMIC_RANGE, 1 };
	//ok = !xioctl(fd, VIDIOC_S_CTRL, &ctrl);
	//if (!ok)
        //        GST_ERROR("WARNING: Unable to set WDR");
	v4l2_control ctrl1 { V4L2_CID_DIGITAL_GAIN, 500 };
	if (xioctl(fd, VIDIOC_S_CTRL, &ctrl1))
		GST_ERROR("WARNING: Unable to set digital gain");
        close(fd);
*/
