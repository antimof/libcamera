...
#define DEFAULT_EXPOSURE		100
#define DEFAULT_ANALOG_GAIN		100
#define DEFAULT_DIGITAL_GAIN	100
...
void gst_libcamera_set_zoom(Rectangle sensor_area, ControlList &controls, const float value)
{
	//std::cout << "Sensor area: " << sensor_area.width << "x" << sensor_area.height << " " << value << "\n";
	int x = (0 + value/10) * sensor_area.width;
	int y = (0 + value/10) * sensor_area.height;
	int w = (1-2*value/10) * sensor_area.width;
	int h = (1-2*value/10) * sensor_area.height;
...
int GstLibcameraSrcState::queueRequest()
{
	...
	if (src_->change_conf) {
		gst_libcamera_set_exposure(controls_,src_->exposure);
		//gst_libcamera_set_shutter_limit(controls_,src_->exposure);
		//gst_libcamera_set_ev(controls_,src_->again);
		gst_libcamera_set_gain(controls_,src_->again);
		gst_libcamera_set_focus(controls_, src_->focus);
		gst_libcamera_set_zoom(src_->sensor_area, controls_, src_->zoom);
		gst_libcamera_set_brightness(controls_, src_->brightness);
		gst_libcamera_set_contrast(controls_, src_->contrast);
		gst_libcamera_set_saturation(controls_, src_->saturation);
		gst_libcamera_set_sharpness(controls_, src_->sharpness);
		src_->change_conf = FALSE;
	}
	{
		MutexLocker locker(controls_lock_);
		wrap->request_.get()->controls() = std::move(controls_);
	}
...
GstLibcameraSrcState::requestCompleted(Request *request)
{
	...
	/*
	std::cout
	<< "Exp: " << request->metadata().get(controls::ExposureTime).value() 
	<< " Again: " << request->metadata().get(controls::AnalogueGain).value() 
	<< " Dgain: " << request->metadata().get(controls::DigitalGain).value()
	//<< " Denoise: " << request->metadata().get(controls::draft::NoiseReductionMode).value()
	<< "\n";
	*/
			     ...
			     gpointer user_data)
{
	...
	gst_libcamera_clamp_and_set_frameduration(state->controls_, state->cam_->controls(), element_caps);
	
	//state->controls_.set(controls::AfModeManual, TRUE);
	state->controls_.set(controls::draft::NoiseReductionMode, controls::draft::NoiseReductionModeEnum::NoiseReductionModeFast);
	//state->controls_.set(controls::draft::NoiseReductionMode, controls::draft::NoiseReductionModeEnum::NoiseReductionModeHighQuality);
	//state->controls_.set(controls::draft::NoiseReductionMode, controls::draft::NoiseReductionModeEnum::NoiseReductionModeOff);
	//state->controls_.set(controls::AeExposureMode, controls::AeExposureModeEnum::ExposureShort);
	self->sensor_area = state->cam_->properties().get(properties::ScalerCropMaximum).value();
	...
	ret = state->cam_->start(&state->controls_);
	if (ret) {
		GST_ELEMENT_ERROR(self, RESOURCE, SETTINGS,
			       ...
			       const GValue *value, GParamSpec *pspec)
{
	...
	case PROP_CAMERA_NAME:
		...
		break;
	case PROP_EXPOSURE:
		self->exposure = g_value_get_int(value);
		self->change_conf = TRUE;
		break;
	case PROP_AGAIN:
		self->again = g_value_get_int(value);
		self->change_conf = TRUE;
		break;
	case PROP_DGAIN:
		self->dgain = g_value_get_int(value);
		gst_libcamera_set_digital_gain(self->dgain);
		break;
	case PROP_FOCUS:
		self->focus = g_value_get_float(value);
		self->change_conf = TRUE;
		break;
	case PROP_ZOOM:
			       ...
			       GParamSpec *pspec)
{
	...
	case PROP_CAMERA_NAME:
		...
		break;
	case PROP_EXPOSURE:
		g_value_set_int(value, self->exposure);
		break;
	case PROP_AGAIN:
		g_value_set_int(value, self->again);
		break;
	case PROP_DGAIN:
		g_value_set_int(value, self->dgain);
		break;
	case PROP_FOCUS:
		g_value_set_float(value, self->focus);
		break;
	case PROP_ZOOM:
...
gst_libcamera_src_class_init(GstLibcameraSrcClass *klass)
{
	...
	spec = g_param_spec_int("exposure", "Exposure",
					       "Set exposure time.", 100, 20000, DEFAULT_EXPOSURE,
					     	(GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
	g_object_class_install_property(object_class, PROP_EXPOSURE, spec);
	
	spec = g_param_spec_int("again", "Analog gain",
					       "Set analog gain.", 0, 200, DEFAULT_ANALOG_GAIN,
					       (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
	g_object_class_install_property(object_class, PROP_AGAIN, spec);
	
	spec = g_param_spec_int("dgain", "Digital gain",
					       "Set digital gain.", 0, 5000, DEFAULT_DIGITAL_GAIN,
					       (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
	g_object_class_install_property(object_class, PROP_DGAIN, spec);
	
	spec = g_param_spec_float("focus", "Lens focus",
					       "Set focus.", 0.0, 5.0, 0.0,
					       (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
	g_object_class_install_property(object_class, PROP_FOCUS, spec);	
	
	spec = g_param_spec_float("zoom", "Digital zoom",
					       "Set zoom.", 0.0, 4.0, 0.0,
					       (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
	g_object_class_install_property(object_class, PROP_ZOOM, spec);	
	
	spec = g_param_spec_float("brightness", "Brightness",
					       "Set brightness.", -1.0, 1.0, 0.0,
					       (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
	g_object_class_install_property(object_class, PROP_BRIGHTNESS, spec);	
	
	spec = g_param_spec_float("contrast", "Contrast",
					       "Set contrast.", 1.0, 4.0, 1.0,
					       (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
	g_object_class_install_property(object_class, PROP_CONTRAST, spec);	
	
	spec = g_param_spec_float("saturation", "Saturation",
					       "Set saturation.", 0.0, 2.0, 0.0,
					       (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
	g_object_class_install_property(object_class, PROP_SATURATION, spec);	
	
	spec = g_param_spec_float("sharpness", "Sharpness",
					       "Set sharpness.", 0.0, 4.0, 0.0,
					       (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
	g_object_class_install_property(object_class, PROP_SHARPNESS, spec);	
}
